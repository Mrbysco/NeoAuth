...
public class MicrosoftUtils {
	...
	public static final String CLIENT_ID = "e16699bb-2aa8-46da-b5e3-45cbcce29091"; //ReAuth Client ID
	...
	public static CompletableFuture<String> acquireMSAuthCode(
		...
		return CompletableFuture.supplyAsync(() -> {
			NeoAuth.LOGGER.info("Acquiring Microsoft auth code...");
			try {
				...
				final HttpServer server = HttpServer.create(
						new InetSocketAddress(NeoAuthConfig.CLIENT.port.get()), 0
				);
				...
				final URIBuilder uriBuilder = new URIBuilder(NeoAuthConfig.CLIENT.authorizeUrl.get())
						.addParameter("client_id", NeoAuthConfig.CLIENT.clientId.get())
				...
				if (prompt != null || NeoAuthConfig.CLIENT.prompt.get() != MicrosoftPrompt.DEFAULT) {
					uriBuilder.addParameter(
							"prompt", (prompt != null ? prompt : NeoAuthConfig.CLIENT.prompt).toString()
					);
				}
				final URI uri = uriBuilder.build();

				// Navigate to the Microsoft login in browser
				NeoAuth.LOGGER.info("Launching Microsoft login in browser: {}", uri.toString());
				browserAction.accept(uri);

				try {
					// Start the HTTP server
					NeoAuth.LOGGER.info("Begin listening on http://localhost:{}/callback for a successful Microsoft login...",
					...
					return Optional.ofNullable(authCode.get())
							...
							.map(code -> {
								NeoAuth.LOGGER.info("Acquired Microsoft auth code! ({})",
				...
				NeoAuth.LOGGER.warn("Microsoft auth code acquisition was cancelled!");
				throw new CancellationException("Interrupted");
			} catch (Exception e) {
				NeoAuth.LOGGER.error("Unable to acquire Microsoft auth code!", e);
	...
	public static CompletableFuture<String> acquireMSAccessToken(final String authCode, final Executor executor) {
		return CompletableFuture.supplyAsync(() -> {
			NeoAuth.LOGGER.info("Exchanging Microsoft auth code for an access token...");
			try (CloseableHttpClient client = HttpClients.createMinimal()) {
				// Build a new HTTP request
				final HttpPost request = new HttpPost(URI.create(NeoAuthConfig.CLIENT.tokenUrl.get()));
				request.setConfig(REQUEST_CONFIG);
				request.setHeader("Content-Type", "application/x-www-form-urlencoded");
				request.setEntity(new UrlEncodedFormEntity(
						List.of(
								new BasicNameValuePair("client_id", NeoAuthConfig.CLIENT.clientId.get()),
								...
								new BasicNameValuePair(
										"redirect_uri",
										String.format("http://localhost:%d/callback", NeoAuthConfig.CLIENT.port.get())
								)
						),
						"UTF-8"
				));
				...
				return Optional.ofNullable(json.get("access_token"))
						...
						// If present, log success and return
						.map(token -> {
							NeoAuth.LOGGER.info("Acquired Microsoft access token! ({})",
				...
				NeoAuth.LOGGER.warn("Microsoft access token acquisition was cancelled!");
				throw new CancellationException("Interrupted");
			} catch (Exception e) {
				NeoAuth.LOGGER.error("Unable to acquire Microsoft access token!", e);
				throw new CompletionException(e);
			}
		}, executor);
	}
	...
	public static CompletableFuture<String> acquireXboxAccessToken(final String accessToken, final Executor executor) {
		return CompletableFuture.supplyAsync(() -> {
			NeoAuth.LOGGER.info("Exchanging Microsoft access token for an Xbox Live access token...");
			try (CloseableHttpClient client = HttpClients.createMinimal()) {
				// Build a new HTTP request
				final HttpPost request = new HttpPost(URI.create(NeoAuthConfig.CLIENT.xboxAuthUrl.get()));
				request.setConfig(REQUEST_CONFIG);
				...
				return Optional.ofNullable(json.get("Token"))
						...
						// If present, log success and return
						.map(token -> {
							NeoAuth.LOGGER.info("Acquired Xbox Live access token! ({})",
									StringUtils.abbreviateMiddle(token, "...", 32));
							return token;
						})
						// Otherwise, throw an exception with the error description if present
				...
				NeoAuth.LOGGER.warn("Xbox Live access token acquisition was cancelled!");
				throw new CancellationException("Interrupted");
			} catch (Exception e) {
				NeoAuth.LOGGER.error("Unable to acquire Xbox Live access token!", e);
				throw new CompletionException(e);
			}
		}, executor);
	}
	...
	public static CompletableFuture<Map<String, String>> acquireXboxXstsToken(
		...
		return CompletableFuture.supplyAsync(() -> {
			NeoAuth.LOGGER.info("Exchanging Xbox Live token for an Xbox Live XSTS token...");
			try (CloseableHttpClient client = HttpClients.createMinimal()) {
				// Build a new HTTP request
				final HttpPost request = new HttpPost(URI.create(NeoAuthConfig.CLIENT.xboxXstsUrl.get()));
				request.setConfig(REQUEST_CONFIG);
				request.setHeader("Content-Type", "application/json");
				request.setEntity(new StringEntity(
						String.format("""
				...
				return Optional.ofNullable(json.get("Token"))
						...
						.map(token -> {
							...
							NeoAuth.LOGGER.info("Acquired Xbox Live XSTS token! (token={}, uhs={})",
				...
				NeoAuth.LOGGER.warn("Xbox Live XSTS token acquisition was cancelled!");
				throw new CancellationException("Interrupted");
			} catch (Exception e) {
				NeoAuth.LOGGER.error("Unable to acquire Xbox Live XSTS token!", e);
				throw new CompletionException(e);
			}
		}, executor);
	}
	...
	public static CompletableFuture<String> acquireMCAccessToken(
		...
		return CompletableFuture.supplyAsync(() -> {
			NeoAuth.LOGGER.info("Exchanging Xbox Live XSTS token for a Minecraft access token...");
			try (CloseableHttpClient client = HttpClients.createMinimal()) {
				// Build a new HTTP request
				final HttpPost request = new HttpPost(URI.create(NeoAuthConfig.CLIENT.mcAuthUrl.get()));
				request.setConfig(REQUEST_CONFIG);
				request.setHeader("Content-Type", "application/json");
				request.setEntity(new StringEntity(
				...
				return Optional.ofNullable(json.get("access_token"))
						...
						// If present, log success and return
						.map(token -> {
							NeoAuth.LOGGER.info("Acquired Minecraft access token! ({})",
									StringUtils.abbreviateMiddle(token, "...", 32));
							return token;
						})
						// Otherwise, throw an exception with the error description if present
				...
				NeoAuth.LOGGER.warn("Minecraft access token acquisition was cancelled!");
				throw new CancellationException("Interrupted");
			} catch (Exception e) {
				NeoAuth.LOGGER.error("Unable to acquire Minecraft access token!", e);
				throw new CompletionException(e);
			}
		}, executor);
	}
	...
	public static CompletableFuture<User> login(final String mcToken, final Executor executor) {
		return CompletableFuture.supplyAsync(() -> {
			NeoAuth.LOGGER.info("Fetching Minecraft profile...");
			try (CloseableHttpClient client = HttpClients.createMinimal()) {
				// Build a new HTTP request
				final HttpGet request = new HttpGet(URI.create(NeoAuthConfig.CLIENT.mcProfileUrl.get()));
				request.setConfig(REQUEST_CONFIG);
				...
				NeoAuth.LOGGER.warn("Minecraft profile fetching was cancelled!");
				throw new CancellationException("Interrupted");
			} catch (Exception e) {
				NeoAuth.LOGGER.error("Unable to fetch Minecraft profile!", e);
				throw new CompletionException(e);
			}
		}, executor);
	}
