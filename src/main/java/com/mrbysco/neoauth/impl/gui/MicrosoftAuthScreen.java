...
public class MicrosoftAuthScreen extends AuthScreen {
	...
	protected void init() {
		...
		addRenderableWidget(cancelBtn = Button.builder(Component.translatable("gui.cancel"), button -> onClose()).bounds(width / 2 - 50, height / 2 + 22, 100, 20).build());
		...
		task = MicrosoftUtils
				...
				.acquireMSAuthCode(success -> Component.translatable("gui.neo_auth.microsoft.browser").getString(), executor, selectAccount ? MicrosoftUtils.MicrosoftPrompt.SELECT_ACCOUNT : null)
